...
This analyses also allows to obtain an overall weighted value of richness change and assess if the mean value of change at a given scale is significantly different from zero.
...
Please note that the 95% confidence interval represented in Fig 1 of Carvalheiro et al (2013) was calculated based on standard error of the log ratio obtained when using the package metafor (In the tables produced by Trend.extractor the column used was ‘Estimated_logRatio_SE’), and after the values (logratio and 95CI) were back-transformed. 


---------------------------------------------------------------------------------------------------------
# Output files from Trend.extractor

##1 - table with corrected estimates of richness change per cell (if no correction was applied, values will be equal to those presented in the table generated by Multilevel.RAR_EXTR)

*Main columns are:*

SCALE - id of spatial scale at which calculations were done, as indicated by the user (e.g. '10x10km cell', 'county', 'state')
TRAIT - id of the trait group for which the calculations were done, as indicated by the user (e.g. 'oligolectic', 'monolectic')
METHOD - this column identifies the Method (Interpolation only, Extrapolation only, Extrapolation up to 3fold + Interpolation) that was used. In Carvalheiro et al. 2013 we opted by Extrap3x (Extrapolation up to 3fold + Interpolation)
CellID - id code of each cell
CORRECTED_logRatio - the estimated  change (natural log of the ratio between estimated richness in post and pre time periods) for the defined cell. To convert to percentage,  use the following formula : 
Change(%)=(exp(logratio)-1)x 100
SElogratio- standard error of the stimated  change (natural log of the ratio between estimated richness in post and pre time periods)
PValue_cell - this column indicates the p-value of the z-test used to check if the estimated change for each spatial scale was significantly different from zero
...
scale -  id of spatial scale at which calculations were done, as indicated by the user (e.g. '10x10km cell', 'county', 'state')
estimated.change - the estimated average percentual change for the defined scale and trait group, calculated based on the average log of the ratio between post period and pre period. To convert to percentage,  the following formula was used: 
Change(%)=(exp(logratio)-1)x 100
weight - this column identifies the weight (no weight, inverse of variance or bootsrapped inverse of the variance) and Method (Interpolation only, Extrapolation only, Extrapolation up to 3fold + Interpolation) that was used. In Carvalheiro et al. 2013 we opted for Ext3x_BOOT_mean_W BootVar.inv (Bootstrapped inverse of variance; Extrapolation up to 3fold + Interpolation)
Nb cells - this column indicates the total number of cells used in the analyses
Nb_cells_decline - this column indicates the total number of cells for which the estimated richness change is negative
Nb_cells_SIGN_decline - this column indicates the number of cells that suffered significant declines
Nb_cells_SIGN_increase - this column indicates the number of cells that suffered significant increases
Seffort P-value- this column indicates the p-value of the z-test used to check if average change for each spatial scale was significant
Estimated_logRatio_SE - this column indicates the standard error of the estimated richness change value (log of the ratio between post period and pre period). To convert to percentage, use the following formula: 
Change(%)=(exp(logratio)-1)x 100

(one day I will adapt the code so that better organized files are generated, with less useless columns) 
