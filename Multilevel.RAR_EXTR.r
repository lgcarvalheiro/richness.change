...
nint<-n
dataset<-rmultinom( repl,sum(vec),vec) # draws 'repl' samples from gridsample, each sample has the number of records of original sample
tau.vector <- 1: repl
tauSD.vector <- 1: repl
...
singletons<-sum(vec==1)
   if( singletons==0)       # if there are no singletons extrapolation formulas do not work
   {tau_n =length(vec)
